"""
Script para crear consultas de ejemplo con enlaces Jitsi
"""
import os
import sys
...
def create_sample_consultations():
    ...
    try:
        ...
        for i, patient in enumerate(patients[:8]):  # Create up to 8 consultations
            ...
            print(f"✅ Created consultation: {patient.full_name} - {specialty} - {scheduled_time.strftime('%Y-%m-%d %H:%M')}")
            print(f"   Jitsi room: {room_name}")
            print(f"   Jitsi URL: {room_url}")
